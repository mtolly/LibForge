...
namespace LibForge.Texture
{
  public class TextureConverter
  {
    ...
    private static byte[] EncodeDxt(Image image, int mapLevel, int nominalWidth = 256, int nominalHeight = 256)
    {
      var img = new Bitmap(image, new Size(nominalWidth >> mapLevel, nominalHeight >> mapLevel));
    ...
    static byte[] HeaderDataAmplitudePS3 = new byte[]
    {
      ...
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x3F, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06,
    ...
    public static Texture ToTextureMaintainingSize(Image image)
    {
      int w = image.Width;
      int h = image.Height;
      int NumMipmaps = 0;
      ...
      Texture.Mipmap[] maps = new Texture.Mipmap[NumMipmaps];
      for (var i = 0; i < maps.Length; i++)
      {
        maps[i] = new Texture.Mipmap
        {
          ...
          Data = EncodeDxt(image, i, image.Width / (1 << i), image.Height / (1 << i))
        };
      }

      return new Texture
      {
        HeaderData = HeaderDataAmplitudePS3,
        FooterData = FooterData256x256,
        Version = 4,
        Mipmaps = maps
      };
